...
class Grid:
    ...
    def _generate_random_grid(self) -> List[List[int]]:
        return [
            [1 if random.random() < self.density else 0 for _ in range(self.cols)]
...
class Game:
    def __init__(self) -> None:
        ...
        self.grid = Grid(density=0.3)  # 30% cells alive initially
    ...
    def _render(self) -> None:
        ...
        pygame.display.set_caption(
            f"Conway's Game of Life — ticks={self.ticks} alive={alive_count} {'(paused)' if self.paused else ''}"
        )
